...
cur.execute("select tab.doc_id from dp_data.dl_acm_documents tab where tab.user_id = " + user_id)
rows = cur.fetchall()
...
cur.execute("select * from dp_data.dl_acm_doc_references tab where tab.doc_id IN " + ids)
reference_rows = cur.fetchall()
print ("Pocet referencii:" + str(len(reference_rows)))

cur.execute("select * from dp_data.dl_acm_doc_authors tab where tab.doc_id IN " + ids)
