...
def test_user(user):
    ...
    metadata = grtMetadata.get_metadata(user)

    metadata_list = []

    for id in text_ids:
        metadata_list.append(metadata[id])
    metadata_list =  sparse.csr_matrix(metadata_list)

    count_vect = CountVectorizer(stop_words='english')
    tfidf_transformer = TfidfTransformer()
    X_train_counts = count_vect.fit_transform(texts)
    X_train_tfidf = tfidf_transformer.fit_transform(X_train_counts)
    x = np.hstack((X_train_tfidf.toarray(),metadata_list.toarray()))
